...
"************RANDOM************",
let $random := for $i in 1 to 500 let $randomPos := random:integer(fn:count($data//*:id)) return $data[$randomPos]/*:id
return (
    stats($random),
    $random
)
...
return(
    ...
    "2 Architectes : " || fn:count($corpus[descendant::*:nbExperts="2"][*:columns = "architecte"]),
    "2 Entrepreneurs : " || fn:count($corpus[descendant::*:nbExperts="2"][*:columns = "entrepreneur"]),
    "Architecte-Entrepreneur : " || fn:count($corpus[descendant::*:nbExperts="2"][*:columns = "architecte entrepreneur"])
)
};
